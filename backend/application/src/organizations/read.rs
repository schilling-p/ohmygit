...
pub async fn list_user_organizations(app_state: AppState user_email: &str) -> Result<Vec<Organization>, AppError> {
    debug!("listing user organizations for: {:?}", user_email);
    let user = retrieve_user_from_db(&pool, UserIdentifier::Email((&user_email).parse::<String>().unwrap())).await?;
    let conn = pool.get().await.map_err(AppError::from)?;
    let orgas = conn
        .interact(move |conn| OrganizationMember::belonging_to(&user.id).inner_join(organizations).select(Organization::as_select()).load(conn))
        .await
